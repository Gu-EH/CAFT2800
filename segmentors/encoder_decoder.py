...
class EncoderDecoder(BaseSegmentor):
    ...
    def extract_feat(self, img):
        ...
        x = self.backbone(img)
        if self.with_neck:       #由于没有在resnet中加入fpn这种neck结构，所以为false
            x = self.neck(x)
        return x
    ...
    def encode_decode(self, img, img_metas):
        ...
        x = self.extract_feat(img)  #如果是fpn的话会输出四张特征图
...
############### loss_decode是个list
        if isinstance(loss_decode, list):
            ...
            losses.update(add_prefix(loss_decode[1], f'aux_{1}'))
            losses.update(add_prefix(loss_decode[2], f'aux_{2}'))
            losses.update(add_prefix(loss_decode[3], f'aux_{3}'))
            losses.update(add_prefix(loss_decode[4], f'aux_{4}'))
            return losses
...
################
    ...
    def simple_test(self, img, img_meta, rescale=True):
        ...
        seg_pred = seg_logit.argmax(dim=1) #shape:torch.Size([1, 512, 512]) 等会要转为512 512 1
        # torch.set_printoptions(threshold=numpy.inf)
        # print(seg_pred)

        # # 顾加thinning算法
        # A = seg_pred.detach().cpu().numpy()  # tensor转换为ndarray
        # A = numpy.array(A,numpy.uint8)  #不转为uint8的话会报错，因为numpy默认是32位的float浮点数而不是uint8  error：(expected: 'processed.type() == CV_8UC1'), where 'processed.type()' is 5 (CV_32FC1)
        # #现在是(c,w,h), 要转为(w,h,c),但实验证明这样还是不行，直接squeeze掉维数为1的那一维
        # # A = numpy.transpose(A,(1,2,0)) 
        # A=numpy.squeeze(A)  #就变成(512，512)了
